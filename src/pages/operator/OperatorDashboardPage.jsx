      ...
      <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold rounded-full uppercase tracking-wider">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <button
            ...
            className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3.5 rounded-2xl font-bold text-sm shadow-md transition-all active:scale-95"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-slate-900">{assignedBatches.length}</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {batches.map((b) => (
              <div
                key={b.id}
                className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-slate-50 transition"
              >
                <div className="space-y-1">
                  ...
                  <h3 className="font-bold text-slate-900 text-sm">{b.formula_name || 'Formula'} ({b.formula_code})</h3>
                  <p className="text-xs text-slate-500">
                    Target Batch Size: <span className="font-bold text-slate-700">{Number(b.target_batch_size).toFixed(2)} kg</span> • Machine: <span className="font-bold text-slate-700">{b.machine_name || 'Mixer MX-01'}</span>
                  </p>
                </div>
