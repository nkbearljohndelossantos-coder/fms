...
def parse_and_import():
    ...
    with zipfile.ZipFile(excel_path, 'r') as z:
        # 1. Memory-efficient iterparse for sharedStrings.xml
        shared_strings = []
        if 'xl/sharedStrings.xml' in z.namelist():
            print("Parsing shared strings with iterparse...")
            sys.stdout.flush()
        ...
        for sheet_name, sheet_path in sheet_files:
                        ...
                        if row_num >= 4:
                            ...
                            if item_name:
                                ...
                                if not any(w in item_upper for w in ['ITEM /CATEGORY', 'ITEM/CATEGORY', 'PRODUCT STICKER', 'TOTAL ESTIMATED COST', 'INITIAL STOCK', 'ACTUAL STOCK']):
                                    ...
                                    storage = cells.get(6, cells.get(5, 'Warehouse Main'))
            ...
            print(f"  -> Sheet '{sheet_name}': Extracted {len(sheet_items)} valid items.")
            sys.stdout.flush()
            all_extracted_items.extend(sheet_items)

        print(f"\n==================================================")
        print(f"TOTAL VALID ITEMS EXTRACTED ACROSS ALL SHEETS: {len(all_extracted_items)}")
        print(f"==================================================")
        sys.stdout.flush()

        # 3. Connect to SQLite and insert/update
        ...
        for item in all_extracted_items:
            ...
            storage = item['storage'] or 'RM-WH-A'
            price = item['price']
            stock = item['stock']
            uom = item['uom']
            ...
            if m_row:
                material_id = m_row[0]
                cursor.execute("UPDATE materials SET cost = ?, vendor_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", (price, vendor_id, material_id))
                updated_materials += 1
            else:
                code_counter += 1
                m_code = f"RM-EXP-{code_counter}"
                uom_cat = 'MASS' if uom in ['kg', 'g'] else 'COUNT'
                cursor.execute("""
                    INSERT INTO materials (code, name, vendor_id, uom, uom_category, cost, currency_code, is_inventoried, is_active, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, 'PHP', 1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                """, (m_code, name, vendor_id, uom, uom_cat, price))
            ...
            if inv_row:
                ...
                cursor.execute("""
                    ...
                    SET current_stock = ?, available_stock = ?, uom = ?, location = ?, vendor_id = ?, status = 'NORMAL', updated_at = CURRENT_TIMESTAMP
                    WHERE id = ?
                """, (stock, stock, uom, storage, vendor_id, inv_id))
                updated_inventory += 1
            else:
                cursor.execute("""
                    ...
                    VALUES ('RAW_MATERIAL', ?, ?, ?, ?, 0, ?, 0, 0, ?, ?, 'NORMAL', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                """, (material_id, lot_number, vendor_id, stock, stock, uom, storage))
        ...
        print(f"\n==================================================")
        print(f"DATABASE RE-INVENTORY COMPLETE:")
        print(f"  - New Materials Created: {inserted_materials}")
        print(f"  - Existing Materials Costs Updated: {updated_materials}")
        print(f"  - New Vendors Created: {inserted_vendors}")
        print(f"  - New Inventory Stock Lots Created: {inserted_inventory}")
        print(f"  - Inventory Stock Lots Updated: {updated_inventory}")
        print(f"==================================================\n")
